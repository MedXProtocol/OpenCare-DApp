...
export const DoctorSelect = connect(mapStateToProps)(withSaga(saga, { propTriggers: ['doctorCount', 'DoctorManager', 'AccountManager']})(class _DoctorSelect extends Component {
  render () {
    const options = this.props.doctors.map(doctor => ({ label: doctor.name, value: doctor.address, publicKey: doctor.publicKey }))
